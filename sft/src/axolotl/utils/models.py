...
def load_model(
    ...
    try:
        ...
        if (  # pylint: disable=condition-evals-to-constant)
            ...
            if cfg.mamba:
                from axolotl.mamba.hybrid_wrapper import MambaTransformerHybridModelWrapper
                model = MambaTransformerHybridModelWrapper.from_pretrained(base_model, attn_implementation="flash_attention_2")
                model.save_config(cfg.output_dir)
                model = model.model
            else:
