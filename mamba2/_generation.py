    ...
    if top_k == 1:  # Short-circuit for greedy decoding
        ...
        if top_k > 0:
            ...
            return indices[
                ...
                torch.multinomial(
                    torch.maximum(probs, torch.zeros_like(probs)), num_samples=1
                ).squeeze(dim=-1),
            ]
        else:
            if min_p > 0.0:
...
def decode(
    ...
    while not should_stop(sequences[-1], inference_params):
        ...
        if eos_token_id is not None:
            sampled_tokens = sampled_tokens.masked_fill(eos_token_mask, eos_token_id)
            eos_token_mask = sampled_tokens == eos_token_id
...
def capture_graph(
    ...
    with torch.cuda.stream(s):
        ...
        # this will cause a error in rl training
