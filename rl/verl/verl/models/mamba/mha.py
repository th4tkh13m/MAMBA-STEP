...
class MHA(nn.Module):
    ...
    def __init__(
        ...
        if not self.cross_attn:
            self.in_proj = wqkv_cls(embed_dim, qkv_dim, bias=qkv_proj_bias, **factory_kwargs)
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        kwargs = (
            {"cu_seqlens": cu_seqlens, "max_seqlen": max_seqlen, "cu_seqlens_k": cu_seqlens, "max_seqlen_k": max_seqlen, **kwargs}
        ...
        if not self.cross_attn and self.num_heads_kv == self.num_heads:
            ...
            if (
                ...
                if self.rotary_emb_dim > 0:
                    qkv = self.rotary_emb(
                        qkv, seqlen_offset=seqlen_offset, cu_seqlens=cu_seqlens, max_seqlen=rotary_max_seqlen,
                    )
                if inference_params is None:
